...
const actions = {
  ...
  adminLogin (store, params) {
    return api.adminLogin(params)
      .then((data) => {
        // saveAccessToken(data.data.data, data.data.token.exp)
        saveAccessToken(data.data.data, 30)
   ...
   * 获取文章列表
   */
  getArticleListToAdmin (store, params) {
    return api.getArticleListToAdmin(params)
      .then((data) => {
        return Promise.resolve(data.data.data)
      })
      .catch((error) => {
